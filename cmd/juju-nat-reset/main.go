...
func (c *NatResetCommand) Info() *cmd.Info {
	return &cmd.Info{
		Name:    "nat",
		Args:    "[args] <target>",
...
func (c *NatResetCommand) SetFlags(f *gnuflag.FlagSet) {
...
func (c *NatResetCommand) Run(ctx *cmd.Context) error {
...
func (c *NatResetCommand) printNatScripts(pending []natcmd.UnitContainment) {
	for _, uc := range pending {
		fmt.Printf("%s:\n", uc.GatewayMachine.Id)
		natcmd.WriteScriptStart(os.Stdout)
		natcmd.WriteScriptReset(os.Stdout)
		fmt.Fprintln(os.Stdout)
	}
}

func (c *NatResetCommand) execNatScripts(pending []natcmd.UnitContainment) {
	for _, uc := range pending {
		var natScript bytes.Buffer
		natcmd.WriteScriptStart(&natScript)
		natcmd.WriteScriptReset(&natScript)
